...
def plot_input_mean(RBM, I, subset, I_range=None,mean=None,weights = None, ax = None, ncols = 1, xlabels = None, with_dots = None , figsize = (3,3), show= True, cc=None):
    ...
    for i in range(nfeatures):
        ...
        ax2_ = ax_.twinx()
        if cc is None:    
            ax2_.hist(I[:,subset[i]],normed=True,weights=weights, bins=100, color = 'gray')
        else:    
            ax2_.hist(I[:,subset[i]],normed=True,weights=weights, bins=100, color = cc)
        ...
        ax_.set_xlabel(xlabels[i],fontsize=16)
        ax_.set_ylabel('Count',fontsize=16)
