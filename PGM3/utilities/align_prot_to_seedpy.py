...
def align_to_seed(seqs_seed, lea, lepmin, lepmax, seqs_new, weights=None):
    ...
    if weights is None:
        alignc_new = eng.balign_prots_seed(seqs_seed, lea, lepmin, lepmax, seqs_new)
    else:
        alignc_new = eng.balign_prots_seed(seqs_seed, lea, lepmin, lepmax, seqs_new, weights) # lea -> final length, lepmin to lepmax -> length included in the profile itself
    eng.quit()
...
import sys
sys.path.append(rootf + '/anaconda3/')
...
np.savetxt(rootf + '/rbm/PGM3/utilities/aligned_prot.txt', seqs_new_n, fmt='%i')
