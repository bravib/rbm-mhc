...
def get_cross_derivatives_Gaussian(V_pos,psi_pos, hlayer, n_cv,weights=None):
    ...
    var_e = average(psi_pos**2,weights=weights) - average(psi_pos,weights=weights)**2
    if n_cv >1:
...
def get_cross_derivatives_ReLU(V_pos,psi_pos, hlayer, n_cv,weights=None):
    ...
    v = p_plus * p_minus * (2*theta/np.sqrt(a) ) * (2*theta/np.sqrt(a) - 1/Phi_plus - 1/Phi_minus)
    ...
    da_db = (dvar_e_db + 0.5 * dmean_v_db * (1+ mean_v + tmp) )/(tmp - dvar_e_da- 0.5 * dmean_v_da * (1+mean_v+tmp))
    da_dtheta = (dvar_e_dtheta + 0.5 * dmean_v_dtheta * (1+ mean_v + tmp) )/(tmp - dvar_e_da- 0.5 * dmean_v_da * (1+mean_v+tmp))
    ...
    if n_cv >1:
        ...
        da_dw = (dvar_e_dw + 0.5 *dmean_v_dw * (1+ mean_v + tmp)[:,np.newaxis,np.newaxis] )/(tmp - dvar_e_da- 0.5 * dmean_v_da * (1+mean_v+tmp))[:,np.newaxis,np.newaxis]
        ...
        da_dw = (dvar_e_dw + 0.5 *dmean_v_dw * (1+ mean_v + tmp)[:,np.newaxis] )/(tmp - dvar_e_da- 0.5 * dmean_v_da * (1+mean_v+tmp))[:,np.newaxis]
...
def get_cross_derivatives_ReLU_plus(V_pos,psi_pos, hlayer, n_cv,weights=None):
    ...
    de_dpsi = 1+v
    de_db = -de_dpsi
    de_da = np.sqrt(a) * (1.0/(2*a*Phi_plus) -(psi_plus-1/Phi_plus)/Phi_plus * dpsi_plus_da)

    dv_dpsi = dpsi_plus_dpsi *  (1+ psi_plus/Phi_plus-1/Phi_plus**2 - (psi_plus-1/Phi_plus)**2  )/Phi_plus
    dv_db = -dv_dpsi
    dv_da = dpsi_plus_da * (1+ psi_plus/Phi_plus-1/Phi_plus**2 - (psi_plus-1/Phi_plus)**2  )/Phi_plus
