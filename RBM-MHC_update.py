'''
January 2025 update of the script RBM-MHC.py. Main updates: updated IEDB loading code to adjust to the new version of IEDB; updated keras classifier module.
'''

rootf='' # please put here the name of your folder
...
def Rand(start, end, num): 
    res = random.sample(range(start,end+1),num)
    return res
...
if yesreal == 0 or makereweighting or cond:
    ...
    hla = list(iedb[head[index_hla]].values)
    antigen = list(iedb[head[index_antigen]].values)
    ...
    condition1 = iedb[head[index_species]].isin(['Homo sapiens', 'Homo sapiens (human)'])
    condition2 = iedb[head[index_class]] == 'I'
    condition3 = iedb[head[index_quality]].isin(quality_pos) 
    condition5 = iedb[head[index_method]].isin(['cellular MHC/mass spectrometry', 'mass spectrometry', 'secreted MHC/mass spectrometry'])
    condition_mon = iedb[head[index_evidence]] == 'Single allele present'
    ...
    condition_ba1 = iedb[head[index_units]] == 'nM'
    condition_ba2 = iedb[head[index_method]].isin(list_meth_ba)
    c_ex11 = iedb[head[index_method]] == 'purified MHC/direct/radioactivity'
    c_ex12 = iedb[head[index_measure]].str.contains('dissociation constant KD') == True
    c_ex21 = iedb[head[index_method]] == 'purified MHC/direct/fluorescence'
    c_ex22 = iedb[head[index_measure]] == 'half maximal effective concentration (EC50)'
    c_ex31 = iedb[head[index_method]] == 'cellular MHC/direct/fluorescence'
    c_ex32 = iedb[head[index_measure]] == 'half maximal effective concentration (EC50)'
    ...
    if makereweighting:
        condition4 = iedb[head[index_hla]].isin(['HLA-A*02:01', 'HLA-A*02:05', 'HLA-A*02:06', 'HLA-A*02:20', 'HLA-A*25:01', 'HLA-A*26:01','HLA-A*29:02', 'HLA-B*08:01', 'HLA-B*14:01', 'HLA-B*14:02', 'HLA-C*03:03', 'HLA-C*07:04'])
        ...
        hla_corr = overlap_seqs(list(np.unique(iedb_selms[head[index_hla]].values)), list(np.unique(iedb_selnms[head[index_hla]].values)))
        # narrow them down to the hlas that have at least 100 seqs in each subset - MS and NON-MS
        hla_corr_100ms = []
        for u in range(len(hla_corr)):
            iedb_sel0 = iedb_selms[iedb_selms[head[index_hla]] == hla_corr[u]]
            if len((iedb_sel0.values)[:,index_antigen]) >= 100:
                hla_corr_100ms.append(hla_corr[u])

        hla_corr_100nms = []
        for u in range(len(hla_corr)):
            iedb_sel0 = iedb_selnms[iedb_selnms[head[index_hla]] == hla_corr[u]]
            if len((iedb_sel0.values)[:,index_antigen]) >= 100:
                hla_corr_100nms.append(hla_corr[u])

        # finally take overlap
        hla_corr_fin = overlap_seqs(hla_corr_100ms, hla_corr_100nms)
        iedb_ms = iedb_selms[iedb_selms[head[index_hla]].isin(hla_corr_fin)]
        iedb_nms = iedb_selnms[iedb_selnms[head[index_hla]].isin(hla_corr_fin)]
...
# load training data (training RBM + training classifier)
if nc == 1 and yesreal == 1:
    ...
    for lcou in range(len(list(np.unique(set_hla)))):
        if lcou == 0:
            l = lcou
        condition4 = iedb[head[index_hla]] == list_hla_l[l]
        if args.ba == 1:
            ...
            ba_chla = (iedb_hla_pos.values)[:,index_baquan]
            f = open(log_file,'a+')
            f.write('Total non-unique data of ' + list_hla_l[l] + ' = ' + str(len(an_chla)) + '\n')
            f.close()
            if len(an_chla) < nminseqAL: # at least 100, otherwise appeal to other techniques
               ...
               ba_chla = (iedb_hla_pos.values)[:,index_baquan]
               f = open(log_file,'a+')
               f.write('in BA less than ' + str(nminseqAL) + ', increase with all techniques to ' + str(len(an_chla))+ '\n')
               f.close()
        else:
            ...
            if len(an_chla) < nminseqMS:
                ...
                tech1 = iedb_hla_pos0[head[index_evidence]].str.contains('Elution') == True
                tech2 = iedb_hla_pos0[head[index_evidence]] == 'Single allele present'
                iedb_hla_pos = iedb_hla_pos0[tech1 | tech2]
                an_chla = (iedb_hla_pos.values)[:,index_antigen]
                f = open(log_file,'a+')
                f.write('in MS monoallelic less than ' + str(nminseqMS) + ', increase with all MS-allele specific purification to ' + str(len(an_chla)) + '\n')
                f.close()
                if len(an_chla) < nminseqMS:
                    iedb_hla_pos = iedb[condition0 & condition1 & condition2 & condition3 & condition4 & condition5]
                    an_chla = (iedb_hla_pos.values)[:,index_antigen]
                    f = open(log_file,'a+')
                    f.write('in all MS-allele specific purification less than ' + str(nminseqMS) + ', increase with all MS to ' + str(len(an_chla)) + '\n')
                    f.close()
                    if len(an_chla) < nminseqAL: # at least 100, otherwise appeal to other techniques
                        iedb_hla_pos = iedb[condition0 & condition1 & condition2 & condition3 & condition4]
                        an_chla = (iedb_hla_pos.values)[:,index_antigen]
                        f = open(log_file,'a+')
                        f.write('in MS less than ' + str(nminseqAL) + ', increase with all techniques ' + str(len(an_chla))+ '\n')
                        f.close()
    ...
    for l in range(len(iedb_data_T)):
        ...
        f = open(log_file,'a+')
        f.write('Final unique data of ' + list_hla_l[l] + ' = ' + str(len(listd)) + ' for average length ' + str(np.mean(range_len)) + '\n')
        f.close()
        if makefig: 
    ...
    # select 10% (perc) of data, those with high confidence for the supervised part (classifier)
    if yesreal:
        filename = out_fold + '/' + args.i + '.txt'
        ms_seqs = []
        with open(filename) as f:
        ...
        for l in range(len(iedb_ind)):
            ...
            limit = round(len(ms_seqs)/(1-fullperc)*fullperc) # amount if labelled data == perc*size dataset
            if len(iedb_ind[l]) > limit:
                for p in range(len(iedb_ind[l])):
                    ...
                    if len(list(np.unique(listi))) >= limit:
                        break
                while len(list(np.unique(listi))) < limit:
                    r = random.randint(0,len(iedb_ind[l])-1)
                    if iedb_qua_fl[iedb_ind[l][r]] == quality_pos[0] or iedb_qua_fl[iedb_ind[l][r]] == quality_pos[1]:
                        listi.append(iedb_ind[l][r]) # high quality data not in ms_seqs
            else:
    ...
    if yesreal:
        ...
        f.write('Amount of total sample = ' + str(len(iedb_data_fl)) + '\n')
        f.close()
    ...
    # Build PWMs from 10% of training data only
...
for runs in range(Nrun):
    ...
    learning_rate = 5e-3 # Initial learning rate (default: 0.1)
    ...
    if maketraining:
        ...
        RBM.fit(all_data.astype(np.int16), weights = all_weights.astype('float32'), batch_size = batch_size, n_iter = n_iter, learning_rate = learning_rate, l1b = l1b, l2_fields = l2f, N_MC = N_MC, decay_after = decay_after, verbose = 1)
    ...
    name_w = out_fold + out_par + '_perc_' + str(perc) + '.weights.h5'
    if maketraining:
        best = keras.callbacks.ModelCheckpoint(name_w, monitor='val_categorical_accuracy', verbose=0, save_best_only=True, save_weights_only=True, mode='auto', save_freq='epoch')
